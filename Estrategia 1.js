...
engine.on('GAME_ENDED', function() {
    ...
    if (recoveryOn && (lastGame.wager > 0) && lastGame.cashedAt == false && (contador < 5 || contador >= 7)){
        putBet *= 2;
        betUnlock = true;
        sangre = false;
    } else if (recoveryOn && (lastGame.wager > 0) && lastGame.cashedAt == false && (contador > 5 && contador < 7)) { //si perdemos muchas seguidas, hacemos descanso
        if (contador == 6) {
            putBet *= 2; //Ya vamos a volver. Aumentamos Martingala
        }
        betUnlock = false;
        sangre = true;
        log ('Río de sangre... vamos a esperar un poco');
        contador += 1;
    } else if (lastGame.wager > 0 && sangre == false){
        betUnlock = false;
    ...
    if (sangre == true) {
        betUnlock = false;
        log ('Rio de sangre... vamos a esperar un poco');
        if (contador == 6) {
            sangre = false;
        }
        contador += 1;
