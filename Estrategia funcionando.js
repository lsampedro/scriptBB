...
function ponerApuesta(apuesta, payout){
    engine.bet(parseFloat(apuesta), parseFloat(payout));
        ...
        log('Apuesta ' + contador + ':- Tamaño Apuesta: '+ apuesta/100 +' payout: '+ payout + 'x');
}
...
engine.on('GAME_STARTING', function() {
    ...
    if (OktoGo && betUnlock){
        ...
        apuesta = Math.floor(putBet / 100) * 100;
        ponerApuesta (apuesta, putMultiplier);
       // engine.bet(parseFloat(apuesta), parseFloat(putMultiplier));
       // log('¡Estás listo para empezar! ');
